...
The original implementation had an issue where .moletracker files received via AirDrop would save to Files instead of opening in the app, and when manually opened, would show an error "Die Datei konnte nicht geöffnet werden" (The file could not be opened).
...
### 1. Updated MoleTrackerApp.swift
...
- `CFBundleDocumentTypes` - Registers .moletracker file type
...
<array>
    <dict>
        ...
        <string>MoleTracker Sync Package</string>
        ...
        <array>
            <string>com.moletracker.sync-package</string>
        </array>
    </dict>
</array>
```
...
- User can browse Files app and select .moletracker files
...
1. User AirDrops .moletracker file
2. iOS recognizes file type and shows "Open in MoleTracker"
...
1. User saves .moletracker file to Files app
2. Opens MoleTracker app
3. Menu → "Import Sync Package"
4. File picker opens
5. User selects .moletracker file
...
3. **Test AirDrop:**
   ...
   - AirDrop to device B
   - Should show "Open in MoleTracker"
   - Tap to open
   - Should see ImportConfirmationView

4. **Test Manual Import:**
   - Save .moletracker file to Files
   - Open MoleTracker
...
- [ ] AirDrop shows "Open in MoleTracker"
...
### Issue: "Open in MoleTracker" doesn't appear
**Solution:** 
...
- Verify file is valid .moletracker package
...
1. Properly registering the .moletracker file type
