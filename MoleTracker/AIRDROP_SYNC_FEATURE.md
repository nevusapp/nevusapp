...
3. Create sync package (.moletracker file)
4. Share via AirDrop
  ↓
...
Sync packages use the `.moletracker` file extension and contain:
...
2. Accept the `.moletracker` file
3. MoleTracker automatically opens
...
#### 6. MoleTrackerApp Integration (`MoleTracker/MoleTrackerApp.swift`)
- `.onOpenURL` handler for `.moletracker` files
