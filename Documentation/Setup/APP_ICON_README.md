...
The Nevus app icon features a **brown mole under a magnifying glass**, symbolizing the app's core functionality of tracking and examining moles on the body.
...
1. **Edit the generator script**:
   ```bash
   open Nevus/IconGenerator.swift
   ```
...
3. **Run the generator**:
   ```bash
   swift Nevus/IconGenerator.swift
   ```

4. **Clean Xcode build** (to refresh icon cache):
   - In Xcode: Product → Clean Build Folder (⇧⌘K)
   - Or via terminal: `rm -rf ~/Library/Developer/Xcode/DerivedData/Nevus-*`
